...
def extendAlcoholVar(xList):
    ...
    return (xExtended, alchCol)


def plotExtendAlcholVar(xList, xExtended, alchCol):

    nrows = len(xList)
    v1 = [xExtended[j][alchCol]-1 for j in range(nrows)]
    
    for i in range(4):
        v2 = [xExtended[j][alchCol-1+i] for j in range(nrows)]
        plot.scatter(v1, v2)
    plot.xlabel("Alcohol")
...
xExtended, alchCol = extendAlcoholVar(xList)

plotExtendAlcholVar(xList, xExtended, alchCol)
